    ...
    public class HttpFactory {
        ...
        public async Task<byte[]> DownloadAsync(string uri, CancellationToken? ct = null, IProgress<double> progress = null) {
            try {
                ...
                using var contentStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false);
                using var memoryStream = contentLength.HasValue ? new MemoryStream(new byte[(int)contentLength.Value], 0, (int)contentLength.Value, true, true) : new MemoryStream((int)Math.Pow(1024, 2) * 10);
                var bufferSize = contentLength.HasValue && contentLength.Value > Math.Pow(1024, 2) * 10 ? 131072 : 32768;
                ...
                while ((chunk = await contentStream.ReadAsync(buffer, 0, buffer.Length, ct ?? CancellationToken.None).ConfigureAwait(false)) > 0) {
                    ...
                    await memoryStream.WriteAsync(buffer, 0, chunk, ct ?? CancellationToken.None).ConfigureAwait(false);
                }
        ...
        private static string GetExtensionFromMimetype(string mimeType) {
            ...
            if (mimeType == "application/x-world")
                return ".vrcw";
            if (mimeType == "application/x-avatar")
                return ".vrca";
            ...
            if (mimeType == "image/png")
                return ".png";
            if (mimeType == "application/x-rsync-signature")
                return ".sig";
            if (mimeType == "application/x-rsync-delta")
                return ".delta";
            if (mimeType == "youtotallydidnt/paste")
                return ".omegalul";
